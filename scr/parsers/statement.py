...
class Statement:
    def __init__(self, statement_str: str, pipline_value: object = None):
        ...
        __STATEMENT_STR__, read only constant keep track of the original statement string
        __pipline_value__, a python object is the value that get piped in this statement
        __evaled_statement_str__, the evaluated document string, this is a variable that changes while we parse
        __para__, a list of parameter type indicates the parameter of the command,
        ...
        self.__para__ = []  # Only useful when uses shell syntax or shell statement to parse
    ...
    def command(self) -> str:
        ...
        only applicable when parse as a shell statement or python in shell syntax
    ...
    def __handle_pipline__(self):
        ...
        1. uses PIPLINE_VALUE_PLACEHOLDER, replace the PIPLINE_VALUE_PLACEHOLDER with the pipline_value
        2. there is a registered parameter that takes pipline, add the parameter name and pipline_value at the end
        3. if there is non of above, then add the pipline_value to the first position as positional parameter
        """
        # first case there is a pipline value placeholder
        if not_in_string_context(substring=PIPLINE_VALUE_PLACEHOLDER,
                                 statement_string=self.__evaled_statement_str__):

            self.__evaled_statement_str__ = \
                self.__evaled_statement_str__.replace(PIPLINE_VALUE_PLACEHOLDER,
        ...
        # second case need to find the command's pipline parameter
            ...
            # using pipline parameter is not supported in python statement and empty statement
            self.__parse_failures__.append(
                ParseFailure(parse_method=ParseMethod.empty_statement,
                             failure_type="PipLineError",
                             failure_message="cannot pip to empty statement"))

            self.__parse_failures__.append(
                ParseFailure(parse_method=ParseMethod.python_statement,
                             failure_type="PipLineError",
                             failure_message="for python statement, you need to indicate where the value get piped to "
                                             "via {0}".format(PIPLINE_VALUE_PLACEHOLDER)))
            try:
                # try to find the data base to get the registered pipline name
                para_name, is_long_name = pipline_helper.get_pipline_para_name(command=self.__command__)
                ...
                arg_list.insert(1, "{0}(self.__pipline_value__)".format(EMBED_MARKER))
