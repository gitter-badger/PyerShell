...
class TestHandlePipline:
    @staticmethod
    def __easy_pip__(statement_str: str, pipline_value: object) -> str:
        """
        this is a helper to construct a statement and then run handle_pipline,
        then get the result
        ...
        test_statement = Statement(
            ...
            pipline_value=pipline_value)
        test_statement.__handle_pipline__()
        return test_statement.__evaled_statement_str__

    def test_pipline_marker(self):
        """
        test when people uses PIPLINE_VALUE_PLACEHOLDER
        """
        result = self.__easy_pip__(
            statement_str="[$_ + i for i in range(2)]",
            pipline_value=1)
    ...
    def test_positional_para(self):
        """
        test to handle pipline by add the pipline_value to the
